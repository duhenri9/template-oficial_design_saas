...
export default function SignupPage() {
  ...
  function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault();
    setLoading(true);
    // DEMO: sem backend. Apenas simula e mostra aviso.
    setTimeout(() => {
      setLoading(false);
      alert("DEMO: Tela de cadastro não funcional. Conecte seu provedor de auth aqui.");
    }, 700);
  }
      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-2xl border-2 border-gray-200 p-8 shadow-sm">
          ...
          <form onSubmit={onSubmit} className="space-y-4">
            ...
            >
              {loading ? "Criando..." : "Criar conta (DEMO)"}
            </button>
          </form>
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            ...
            <Link href="/login" className="hover:text-indigo-600 font-medium">Já tenho conta</Link>
          </div>
        </div>
      </div>
    </div>
